...
| `space /` | search.workspace | Adapter | P1 | Live grouped QuickPick workspace search showing query, file groups, line/column, and line preview. |
...
Current MVP uses a concise status bar prefix indicator plus a VS Code QuickPick prefix popup populated from the pure core prefix table. Typing an exact key match while the popup is open dispatches immediately for normal prefixes; arrow-key navigation and Enter remain available for discovery. `:` command mode is different: typed text is shown in the picker and commands only execute after Enter, so longer commands like `:wqa!` are possible without `:w` short-circuiting. Nested prefix hints are supported for match/surround flows such as `m s` and `m r (`. Dismissing the popup clears pending prefix state. A custom Wisp-style webview/decorated picker remains deferred.
