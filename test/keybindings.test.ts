...
describe('extension keybindings', () => {
  ...
  test('captures escape parity, command prefix, and selection collapse keys', () => {
    ...
    expect(has('shift+w', 'W')).toBe(true)
    expect(has('shift+r', 'R')).toBe(true)
    expect(has('shift+i', 'I')).toBe(true)
    expect(has('shift+a', 'A')).toBe(true)
    expect(has('shift+o', 'O')).toBe(true)
    expect(has('f', 'f')).toBe(true)
    expect(has('shift+f', 'F')).toBe(true)
    expect(has('t', 't')).toBe(true)
    expect(has('shift+t', 'T')).toBe(true)
